...
class App_modules
{
    ...
    public function upload()
    {
        ...
        if (isset($_FILES['module']['name'])) {
            ...
            $tmpFilePath = $_FILES['module']['tmp_name'];

            // Make sure we have a filepath
            $zip = new ZipArchive;
            if ($zip->open($tmpFilePath) === true) {
                if (!$zip->extractTo(APP_MODULES_PATH)) {
                    $response['error'] = 'Failed to extract module';
                } else {
                ...
                $response['error'] = 'Failed to open the .zip file';
            }
    ...
    public function initialize()
    {
        ...
        foreach ($this->get_valid_modules() as $module) {
            ...
            if (!isset($this->modules[$name])) {
                 ...
                 * Module headers
                 */
                $this->modules[$name]['headers'] = $this->get_headers($name);
                /**
                 * Init file path
     ...
     * @param  string $module Module Name [system_name]
     * @return array
     */
    public function get_headers($module)
    {
        if (isset($this->modules[$module]['headers']) && count($this->modules[$module]['headers']) > 0) {
            return $this->modules[$module]['headers'];
        }

        $module = strtolower(trim($module)); // Lowercase and trim the module name

        $module_data = read_file(APP_MODULES_PATH . $module . '/' . $module . '.php'); // Read the module init file.
