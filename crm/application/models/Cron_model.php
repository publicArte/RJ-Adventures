...
class Cron_model extends CRM_Model
{
        ...
        if (empty($mail->last_run) || (time() > $mail->last_run + ($mail->check_every * 60))) {
            ...
            foreach ($emails as $email) {
                ...
                if ($mailstatus) {
                    ...
                    logActivity('Lead Email Integration Blocked Email by Spam Filters [' . $mailstatus . ']');
    ...
    // Remove temporary files older than 30 minutes
    private function maybe_delete_old_temporary_files()
    {
        $older_than = hooks()->apply_filters('delete_old_temporary_files_older_than', 1800);
