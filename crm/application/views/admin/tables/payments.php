...
if (!has_permission('payments', '', 'view')) {
    $whereUser = '';
    $whereUser .= 'AND (invoiceid IN (SELECT id FROM ' . db_prefix() . 'invoices WHERE (addedfrom=' . get_staff_user_id() . ' AND addedfrom IN (SELECT staffid FROM ' . db_prefix() . 'staff_permissions JOIN ' . db_prefix() . 'permissions ON ' . db_prefix() . 'permissions.permissionid=' . db_prefix() . 'staff_permissions.permissionid WHERE ' . db_prefix() . 'permissions.name = "invoices" AND can_view_own=1)))';
