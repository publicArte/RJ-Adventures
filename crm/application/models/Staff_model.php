...
class Staff_model extends CRM_Model
{
    ...
    public function get_staff_permissions($id)
    {
        ...
        if (!$permissions && !is_array($permissions)) {
            $this->db->select(db_prefix() . 'staff_permissions.*,' . db_prefix() . 'permissions.shortname as permission_name');
            $this->db->join(db_prefix() . 'permissions', db_prefix() . 'permissions.permissionid = ' . db_prefix() . 'staff_permissions.permissionid');
            $this->db->where('staffid', $id);
            $permissions = $this->db->get(db_prefix() . 'staff_permissions')->result();
    ...
    public function update_permissions($permissions, $id)
    {
        ...
        if (total_rows(db_prefix() . 'staff_permissions', [
            'staffid' => $id,
        ]) == 0) {
            foreach ($all_permissions as $p) {
                ...
                $this->db->insert(db_prefix() . 'staff_permissions', $_ins);
            }
        } elseif (total_rows(db_prefix() . 'staff_permissions', [
                'staffid' => $id,
