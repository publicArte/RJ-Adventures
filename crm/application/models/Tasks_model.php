    ...
    public function get_staff_members_that_can_access_task($task_id)
    {
        return $this->db->query('SELECT * FROM ' . db_prefix() . 'staff
            WHERE (
                    ...
                    OR staffid IN(SELECT staffid FROM " . db_prefix() . 'staff_permissions JOIN ' . db_prefix() . 'permissions ON ' . db_prefix() . 'permissions.permissionid=' . db_prefix() . 'staff_permissions.permissionid WHERE ' . db_prefix() . 'permissions.name = "tasks" AND can_view=1)
                )
